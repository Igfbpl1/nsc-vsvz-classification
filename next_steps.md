...
- Positive computational findings not in the published paper:
  ...
  2. **Gjc3 is the #1 OL velocity driver** (corr 214.77), with Fa2h a top-10 COP driver (rank 7, corr 35.54) — the strongest transcriptional-momentum signals toward OL maturation localize to specific genes at the COP→OL stages. Both are also SHAP POSITIVE_OL (Gjc3 rank 8, Fa2h rank 9): two independent methods converge.
  3. **Meis2 climbs from TAP velocity rank 5 (Cntl) to rank 3 (CupRap)** (corr 72.47, Spearman 0.93) — a neuroblast gene whose velocity signal *rises* in CupRap TAPs. Meis2 is SHAP rank 4 NEGATIVE_OL, so this is CupRap TAPs being pushed toward NB fate, not OL — unexpected and not reported by Willis et al.
  4. **aNSC fate-bias signal under acute injury** — aNSC `bias` median shifts from -0.07 (Control) to +0.15 (CupRap NoRecov), Mann-Whitney p = 2.9 × 10⁻¹⁴, with full return to baseline by 3 weeks (p = 0.77 vs Control). Resolving aNSCs as their own category (paper-aligned split from dNSC) makes this activation→OL-tilt signal cleanly measurable.
