...
def qc_filter(
    ...
    if failed_min_genes_adata.n_obs > 0:
        print(
            f"  Top 10 cells failing min_genes ({min_genes}): {list(failed_min_genes_adata.obs_names[:10])}"
        )
        if output_dir:
            # Save obs_names, n_genes_by_counts, and pct_counts_mt for the top 10 failing cells
            df_to_save = failed_min_genes_adata.obs[
                ["n_genes_by_counts", "pct_counts_mt", "total_counts"]
            ].head(10)
    ...
    if failed_max_pct_mt_adata.n_obs > 0:
        print(
            f"  Top 10 cells failing max_pct_mt ({max_pct_mt}%): {list(failed_max_pct_mt_adata.obs_names[:10])}"
        )
        if output_dir:
            # Save obs_names, n_genes_by_counts, and pct_counts_mt for the top 10 failing cells
            df_to_save = failed_max_pct_mt_adata.obs[
                ["n_genes_by_counts", "pct_counts_mt", "total_counts"]
            ].head(10)
            df_to_save.index.name = "barcode"
            df_to_save["min_genes_threshold"] = min_genes
            ...
            df_to_save["min_cells_threshold"] = min_cells
            df_to_save.to_csv(
                f"{output_dir}/failed_max_pct_mt_cells_metadata_top10.csv"
            )
    ...
    if len(filtered_genes_names) > 0:
        print(
            f"  Top 10 genes failing min_cells ({min_cells}): {filtered_genes_names[:10]}"
        )
        if output_dir:
            # Re-calculate n_cells for genes on the current adata to identify those that would be filtered
            temp_adata_for_gene_filter = (
                adata.copy()
            )  # Make a temporary copy to avoid modifying adata again
            sc.pp.calculate_qc_metrics(
                temp_adata_for_gene_filter,
                qc_vars=[],
                inplace=True,
            ...
            failed_min_cells_mask = (
                temp_adata_for_gene_filter.var["n_cells_by_counts"] < min_cells
            )
            failed_min_cells_adata = temp_adata_for_gene_filter[
                :, failed_min_cells_mask
            ]
            ...
            if failed_min_cells_adata.n_vars > 0:
                ...
                df_to_save["min_cells_threshold"] = min_cells
                df_to_save.to_csv(
                    f"{output_dir}/failed_min_cells_genes_metadata_top10.csv"
                )
